...
//     // Add user message (no changes here)
//     setMessages((prev) => [
//       ...prev,
//       {
//         text: message,
...
//     // Start thinking indicator (no changes here)
//     setIsThinking(true);
//     setThinkingText("");
//     let dots = 0;
...
//     try {
//       // ✅ NEW: Only include session_id if it exists
//       const payload: any = {
//         query: message,
//         user_id: "67f49b5544c55c0013794912",
//       };
//       if (sessionId) payload.session_id = sessionId;

//       const response = await fetch("http://192.168.1.18:5000/api/chat", {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
//         },
//         body: JSON.stringify(payload), // Use the safe payload!
//       });
...
export const ChatBot = () => {
  ...
  const handleReactToMessage = async (
    ...
    try {
      await fetch("http://192.168.1.18:5000/api/chat/rate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const clearConversation = (force?: boolean | React.MouseEvent) => {
    ...
    if (sessionId) {
      fetch("http://192.168.1.18:5000/api/memory/clear", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ session_id: sessionId }),
      })
